...
class Monomers:
    ...
    def Wall_time(self):
        ...
        #for i in range(self.NM):
         #   if self.vel[i][0] == 0:
          #      dt_List.append([np.inf, np.inf])
           # else:
            #    dt_List.append((coll_condition[i] - self.pos[i])/self.vel[i])
    ...
    def Mono_pair_time(self):
       ...
       # dr_list = self.pos[mono_i]-self.pos[mono_j]
        #dv_list = self.vel[mono_i]-self.vel[mono_j]
    
       # sumRad_list = self.rad[mono_i]+self.rad[mono_j]

        #a = dv_list[:,0]**2 + dv_list[:,1]**2
        #b = 2*(dv_list[:,0]*dr_list[:,0] + dv_list[:,1]*dr_list[:,1])
        #c = dr_list[:,0]**2 + dr_list[:,1]**2 - (sumRad_list[:])**2
        #delta= b**2 - 4*a*c
       # minCollTime = np.where( (delta>0) & (b<0) , (1/(2*a))*(- b - np.sqrt(delta)))
        #collision_disk_1 = minCollTime // 2
        #collision_disk_2 = 1 
       ...
       # self.next_mono_coll.mono_1 = collision_disk_1
        ...
        dt= np.where(((delta>0)&(b<0)), (1/(2*a))*(-b-np.sqrt(delta)),np.inf)
        
        min_index=np.argmin(dt)
        
        collision_disk_1, collision_disk_2 = self.mono_pairs[min_index]
        
        
        self.next_mono_coll.dt = dt[min_index]
        self.next_mono_coll.mono_1 = collision_disk_1
        self.next_mono_coll.mono_2 = collision_disk_2
        #self.next_mono_coll.w_dir = not necessary
    ...
    def compute_new_velocities(self, next_event):
        ...
        if next_event.Type=='wall':
            ...
            delta = self.pos[mono_2] - self.pos[mono_1]
            delta=delta/np.linalg.norm(delta)
            m1=self.mass[mono_1]
            m2=self.mass[mono_2]
            Dv=self.vel[mono_1]-self.vel[mono_2]
            self.vel[mono_1]=self.vel[mono_1] - (2*m2)/(m1+m2)*np.inner(delta,Dv)*delta
            print((2*m2)/(m1+m2)*np.inner(delta,Dv)*delta)
            self.vel[mono_2]=self.vel[mono_2] + (2*m1)/(m1+m2)*np.inner(delta,Dv)*delta
